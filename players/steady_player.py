...
class SteadyPlayer(BasePokerPlayer):
    """Jogador estável e consistente. Mantém estratégia sem mudanças bruscas. Usa sistema de memória unificado."""
    
    def __init__(self, memory_file="steady_player_memory.json"):
        # Inicializa gerenciador de memória unificada
        self.memory_manager = UnifiedMemoryManager(
            memory_file,
            default_bluff=0.16,  # Estável
            default_aggression=0.53,  # Estável
            default_tightness=27  # Estável
        )
    ...
    def _should_bluff(self):
        """Decide se deve blefar - estável."""
        return random.random() < self.bluff_probability
    
    def _bluff_action(self, valid_actions, round_state):
        """Blefe estável: consistente."""
        if valid_actions[2]['amount']['min'] != -1 and random.random() < 0.48:
    ...
    def _normal_action(self, valid_actions, hand_strength, round_state, current_actions=None, bluff_analysis=None):
        """Ação estável: consistente, considerando ações atuais."""
        ...
        # Mão muito forte: raise estável
        if hand_strength >= 50:
            raise_action = valid_actions[2]
            if raise_action['amount']['min'] != -1:
                min_amount = raise_action['amount']['min']
                max_amount = raise_action['amount']['max']
                amount = random.randint(min_amount, min(max_amount, min_amount + 17))
                return raise_action['action'], amount
        ...
        if hand_strength >= adjusted_threshold:
            ...
            if adjusted_aggression > 0.53 and valid_actions[2]['amount']['min'] != -1:
                return valid_actions[2]['action'], valid_actions[2]['amount']['min']
            else:
                call_action = valid_actions[1]
                return call_action['action'], call_action['amount']
        ...
        if bluff_analysis and bluff_analysis['should_call_bluff']:
            if hand_strength >= 26:  # Steady: paga blefe com mão razoável
                call_action = valid_actions[1]
                return call_action['action'], call_action['amount']
    ...
    def receive_round_result_message(self, winners, hand_info, round_state):
        """Aprendizado estável: ajusta muito gradualmente."""
        ...
        # Aprendizado estável: ajustes muito graduais
        ...
        if len(round_history) >= 15:
            ...
            # Ajustes muito graduais para manter estabilidade
            if win_rate > 0.59:
                self.memory['aggression_level'] = min(0.66, self.memory['aggression_level'] * 1.002)
                self.memory['bluff_probability'] = min(0.19, self.memory['bluff_probability'] * 1.002)
            elif win_rate < 0.33:
                self.memory['tightness_threshold'] = min(30, self.memory['tightness_threshold'] + 1)
                self.memory['aggression_level'] = max(0.44, self.memory['aggression_level'] * 0.998)
