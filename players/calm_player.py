...
class CalmPlayer(BasePokerPlayer):
    """Jogador calmo e controlado. Mantém serenidade mesmo em situações difíceis. Usa sistema de memória unificado."""
    
    def __init__(self, memory_file="calm_player_memory.json"):
        # Inicializa gerenciador de memória unificada
        self.memory_manager = UnifiedMemoryManager(
            memory_file,
            default_bluff=0.14,  # Calmo
            default_aggression=0.49,  # Controlado
    ...
    def _should_bluff(self):
        """Decide se deve blefar - calmo."""
        return random.random() < self.bluff_probability
    
    def _bluff_action(self, valid_actions, round_state):
        """Blefe calmo: controlado."""
        if valid_actions[2]['amount']['min'] != -1 and random.random() < 0.35:
    ...
    def _normal_action(self, valid_actions, hand_strength, round_state, current_actions=None, bluff_analysis=None):
        """Ação calma: controlada, considerando ações atuais."""
        ...
        # NOVO: Campo passivo reduz threshold moderadamente (CalmPlayer é calmo)
        if current_actions and current_actions.get('is_passive', False):
            ...
            adjusted_threshold = max(26, adjusted_threshold - int(passive_score * 3))
        
        # Mão muito forte: raise calmo
        if hand_strength >= 53:
            raise_action = valid_actions[2]
        ...
        # NOVO: Com campo muito passivo, CalmPlayer pode fazer raise com mão forte
        if current_actions and current_actions.get('is_passive', False):
            passive_score = current_actions.get('passive_opportunity_score', 0.0)
            if hand_strength >= 48 and passive_score > 0.7:
                raise_action = valid_actions[2]
                if raise_action['amount']['min'] != -1:
                    return raise_action['action'], raise_action['amount']['min']
        
        # Mão forte: call (calmo)
        ...
        if bluff_analysis and bluff_analysis['should_call_bluff']:
            if hand_strength >= 27:  # Calm: paga blefe com mão razoável
                call_action = valid_actions[1]
                return call_action['action'], call_action['amount']
    ...
    def receive_round_result_message(self, winners, hand_info, round_state):
        """Aprendizado calmo: ajusta muito gradualmente, mantém controle."""
        ...
        # Aprendizado calmo: ajustes muito graduais
        round_history = self.memory.get('round_history', [])
        if len(round_history) >= 18:
            recent_rounds = round_history[-18:]
            win_rate = sum(1 for r in recent_rounds if r['final_result']['won']) / len(recent_rounds)
            
            # Ajustes muito graduais para manter calma
            if win_rate > 0.63:
                ...
                self.memory['aggression_level'] = max(0.38, self.memory['aggression_level'] * 0.999)
