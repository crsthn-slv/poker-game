...
class CautiousPlayer(BasePokerPlayer):
    ...
    def _normal_action(self, valid_actions, hand_strength, round_state, current_actions=None, bluff_analysis=None):
        ...
        # NOVO: Campo passivo reduz threshold moderadamente (CautiousPlayer é muito conservador)
        if current_actions and current_actions.get('is_passive', False):
            ...
            # Reduz threshold apenas um pouco (cauteloso)
            adjusted_threshold = max(30, adjusted_threshold - int(passive_score * 2))
        ...
        # NOVO: Com campo muito passivo, CautiousPlayer pode fazer raise com mão forte
        if current_actions and current_actions.get('is_passive', False):
            passive_score = current_actions.get('passive_opportunity_score', 0.0)
            if hand_strength >= 50 and passive_score > 0.7:
                raise_action = valid_actions[2]
                if raise_action['amount']['min'] != -1:
        ...
        if bluff_analysis and bluff_analysis['should_call_bluff']:
            if hand_strength >= 30:  # Cautious: paga blefe com mão razoável
                call_action = valid_actions[1]
                return call_action['action'], call_action['amount']
