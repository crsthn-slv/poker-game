...
class SteadyAggressivePlayer(BasePokerPlayer):
    ...
    def __init__(self, memory_file="steady_aggressive_player_memory.json"):
        # Inicializa gerenciador de memória unificada
        self.memory_manager = UnifiedMemoryManager(
            memory_file,
            default_bluff=0.18,  # Agressivo mas controlado
            default_aggression=0.59,  # Alto mas não extremo
            default_tightness=26  # Menos seletivo
        )
    ...
    def _should_bluff(self):
        """Decide se deve blefar - agressivo mas controlado."""
        return random.random() < self.bluff_probability
    
    def _bluff_action(self, valid_actions, round_state):
        """Blefe agressivo mas controlado: prefere raise."""
        if valid_actions[2]['amount']['min'] != -1 and random.random() < 0.55:
    ...
    def _normal_action(self, valid_actions, hand_strength, round_state, current_actions=None, bluff_analysis=None):
        ...
        if current_actions:
            if current_actions['has_raises']:
                adjusted_threshold += 4 + (current_actions['raise_count'] * 2)
        ...
        # Mão fraca: fold apenas se for MUITO fraca (agressivo mas controlado)
    ...
    def receive_round_result_message(self, winners, hand_info, round_state):
        """Aprendizado agressivo mas controlado: ajusta baseado em resultados."""
        ...
        # Aprendizado agressivo mas controlado
        ...
        if len(round_history) >= 10:
            ...
            # Ajustes agressivos mas controlados
            if win_rate > 0.58:
                self.memory['aggression_level'] = min(0.72, self.memory['aggression_level'] * 1.005)
                self.memory['bluff_probability'] = min(0.22, self.memory['bluff_probability'] * 1.005)
            elif win_rate < 0.32:
                self.memory['tightness_threshold'] = min(29, self.memory['tightness_threshold'] + 1)
                self.memory['aggression_level'] = max(0.48, self.memory['aggression_level'] * 0.995)
                self.memory['bluff_probability'] = max(0.14, self.memory['bluff_probability'] * 0.995)
