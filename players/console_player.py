...
class ConsolePlayer(BasePokerPlayer):
    ...
    def __init__(self, input_receiver=None, initial_stack=100, small_blind=None, big_blind=None, show_win_probability=False, printer=print):
    ...
    def set_uuid(self, uuid):
        ...
        if fixed_uuid:
            ...
            if debug_mode:
                self.printer(f"[DEBUG] ConsolePlayer UUID: PyPokerEngine={uuid} -> Fixed={fixed_uuid} (You)")
        else:
    ...
    def declare_action(self, valid_actions, hole_card, round_state):
        ...
        if hole_cards:
            ...
            if player_uuid_for_storage:
                ...
                if os.environ.get('POKER_DEBUG', 'false').lower() == 'true':
                    ...
                    self.printer(f"[DEBUG] Cartas armazenadas: uuid={player_uuid_for_storage}, hole_cards={hole_cards}, registry_has={player_uuid_for_storage in all_cards_after}")
        ...
        if hole_cards:
            ...
            if self.show_win_probability:
                try:
                    ...
                    if debug_mode:
                        self.printer(f"[DEBUG] Iniciando cálculo de probabilidade - hole_cards={hole_cards}, street={current_street}")
                    ...
                    if hasattr(self, 'uuid') and self.uuid:
                        ...
                        if debug_mode:
                            self.printer(f"[DEBUG] UUID obtido de self.uuid: {player_uuid}")
                    else:
                        if debug_mode:
                            self.printer(f"[DEBUG] self.uuid não disponível, tentando obter do round_state...")
                            ...
                            if isinstance(seat, dict):
                                ...
                                if seat_hole_card and hole_cards:
                                    ...
                                    if seat_hole_cards == hole_cards:
                                        ...
                                        if debug_mode:
                                            self.printer(f"[DEBUG] UUID encontrado no round_state: {player_uuid}")
                                        # Armazena para uso futuro
                                        if not hasattr(self, 'uuid'):
                                            self.uuid = player_uuid
                                        break
                        if not player_uuid and debug_mode:
                            self.printer(f"[DEBUG] UUID não encontrado no round_state!")
                    
                    if player_uuid:
                        if debug_mode:
                            self.printer(f"[DEBUG] Calculando probabilidade para uuid={player_uuid}")
                        # Gera chave de cache
                        ...
                        if cache_key in self.win_probability_cache:
                            ...
                            if debug_mode:
                                self.printer(f"[DEBUG] Usando cache: {win_probability_display}")
                        else:
                            if debug_mode:
                                self.printer(f"[DEBUG] Cache miss, calculando nova probabilidade...")
                            # Calcula nova probabilidade
                            # No preflop, retorna intervalo de confiança
                            if current_street == 'preflop':
                                if debug_mode:
                                    self.printer(f"[DEBUG] Chamando calculate_win_probability_for_player (preflop)...")
                                ...
                                if debug_mode:
                                    self.printer(f"[DEBUG] Resultado: {win_prob_data}")
                                if win_prob_data is not None:
                                    ...
                                    if debug_mode:
                                        self.printer(f"[DEBUG] Probabilidade formatada: {win_probability_display}")
                                    ...
                                    self.printer(f"[DEBUG] calculate_win_probability_for_player retornou None!")
                            else:
                                # Em outras streets, mostra apenas o valor único
                                if debug_mode:
                                    self.printer(f"[DEBUG] Chamando calculate_win_probability_for_player (street={current_street})...")
                                ...
                                if debug_mode:
                                    self.printer(f"[DEBUG] Resultado: {win_prob}")
                        ...
                        self.printer(f"[DEBUG] player_uuid é None, não calculando probabilidade")
                except Exception as e:
                    # Log de erro mais informativo
                    debug_mode = os.environ.get('POKER_DEBUG', 'false').lower() == 'true'
                    if debug_mode:
                        self.printer(f"[DEBUG] Erro ao calcular probabilidade: {type(e).__name__}: {e}")
                        import traceback
                        traceback.print_exc()
            ...
            if win_probability_display is not None:
                self.printer(f"Your cards: {cards_display} | Hand: {hand_strength_with_level} | Win probability: {win_probability_display}")
            else:
                # Log temporário para debug - sempre mostra se probabilidade não foi calculada
                import os
                if os.environ.get('POKER_DEBUG', 'false').lower() == 'true':
                    self.printer(f"[DEBUG] win_probability_display é None - probabilidade não calculada")
                self.printer(f"Your cards: {cards_display} | Hand: {hand_strength_with_level}")
        
        # 2. Cartas comunitárias (quando existirem)
        if community_cards:
            community_display = self.formatter.format_cards_display_with_color(community_cards)
            self.printer(f"Community cards: {community_display}")
        ...
        if debug_mode:
            ...
            self.printer(f"[DEBUG] self.uuid: {getattr(self, 'uuid', 'NÃO DEFINIDO')}")
            self.printer(f"[DEBUG] self.initial_stack: {self.initial_stack}")
            self.printer(f"[DEBUG] hole_cards: {hole_cards}")
            self.printer(f"[DEBUG] Seats disponíveis ({len(seats)}):")
            for i, seat in enumerate(seats):
                if isinstance(seat, dict):
                    self.printer(f"[DEBUG]   Seat {i}: name='{seat.get('name')}', uuid={seat.get('uuid')}, stack={seat.get('stack')}, state={seat.get('state')}")
                    if seat.get('hole_card'):
                        seat_cards = normalize_hole_cards(seat.get('hole_card'))
                        self.printer(f"[DEBUG]     hole_card: {seat_cards}")
        ...
        if hasattr(self, 'uuid') and self.uuid:
            for seat in seats:
                if isinstance(seat, dict) and seat.get('uuid') == self.uuid:
                    player_seat = seat
                    if debug_mode:
                        self.printer(f"[DEBUG] ✓ Seat encontrado pelo UUID: {self.uuid}")
                    break
        
        # 2. Se não encontrou, tenta pelo nome do jogador
        target_name = (self._player_name or "You").lower()
        if not player_seat:
            for seat in seats:
                if isinstance(seat, dict) and seat.get('name', '').lower() == target_name:
                    player_seat = seat
                    # Atualiza self.uuid se não estava definido
                    if not hasattr(self, 'uuid') or not self.uuid:
                        self.uuid = seat.get('uuid')
                        if debug_mode:
                            self.printer(f"[DEBUG] ✓ UUID atualizado: {self.uuid}")
                    if debug_mode:
                        self.printer(f"[DEBUG] ✓ Seat encontrado pelo nome 'You', UUID: {seat.get('uuid')}")
                    break
                    ...
                    if seat_hole_card:
                        ...
                        if seat_hole_cards == hole_cards:
                            ...
                            if not hasattr(self, 'uuid') or not self.uuid:
                                self.uuid = seat.get('uuid')
                                if debug_mode:
                                    self.printer(f"[DEBUG] ✓ UUID atualizado pelas cartas: {self.uuid}")
                            if debug_mode:
                                self.printer(f"[DEBUG] ✓ Seat encontrado pelas cartas, UUID: {seat.get('uuid')}")
                            break
        ...
        if player_seat:
            ...
            if debug_mode:
                self.printer(f"[DEBUG] ✓ Stack encontrado: {my_stack} (initial_stack: {self.initial_stack})")
                if my_stack == 0:
                    self.printer(f"[DEBUG] ⚠️  ATENÇÃO: Stack é 0! Isso pode indicar um problema.")
        else:
            if debug_mode:
                self.printer(f"[DEBUG] ❌ ERRO: Seat do jogador não encontrado!")
                self.printer(f"[DEBUG] self.uuid: {getattr(self, 'uuid', 'NÃO DEFINIDO')}")
                self.printer(f"[DEBUG] Tentou buscar por: UUID, nome 'You', e cartas")
            # Fallback: tenta usar o primeiro seat se houver apenas um (caso edge)
            if len(seats) == 1 and isinstance(seats[0], dict):
                player_seat = seats[0]
                my_stack = player_seat.get('stack', 0)
                if debug_mode:
                    self.printer(f"[DEBUG] ⚠️  Fallback: usando único seat disponível, stack: {my_stack}")
        
        if debug_mode:
            self.printer(f"[DEBUG] === Fim da busca ===\n")
        ...
        self.printer(f"Pot {pot_display} | Your chips {stack_display}")
        ...
        if action_line_parts:
            self.printer(f"Available actions: {' | '.join(action_line_parts)}")
    ...
    def receive_street_start_message(self, street, round_state):
        ...
        if self.pot_line_printed:
            ...
            if self.pot_updates:
                ...
                self.printer(pot_line)  # Quebra linha após mostrar todas as atualizações
            else:
                self.printer()  # Quebra linha
            self.pot_line_printed = False
    ...
    def receive_game_update_message(self, new_action, round_state):
        ...
        if self.game_history:
            ...
            if debug_mode:
                self.printer(f"[HISTORY DEBUG] pypoker_uuid from new_action: {pypoker_uuid[:8] if pypoker_uuid else 'None'}")
            ...
            if pypoker_uuid:
                for seat in seats:
                    if isinstance(seat, dict) and seat.get('uuid') == pypoker_uuid:
                        ...
                        if debug_mode:
                            self.printer(f"[HISTORY DEBUG] ✓ Found seat: name='{seat_name}', fixed_uuid={fixed_player_uuid[:8] if fixed_player_uuid else 'None'}")
                        break
            
            if not fixed_player_uuid and debug_mode:
                self.printer(f"[HISTORY DEBUG] ❌ Could not find seat for pypoker_uuid: {pypoker_uuid[:8] if pypoker_uuid else 'None'}")
                self.printer(f"[HISTORY DEBUG] Available seats:")
                for i, seat in enumerate(seats):
                    if isinstance(seat, dict):
                        self.printer(f"[HISTORY DEBUG]   Seat {i}: uuid={seat.get('uuid', '')[:8] if seat.get('uuid') else 'None'}, name='{seat.get('name', '')}'")
            ...
            if fixed_player_uuid:
                ...
                if hasattr(self, 'uuid') and self.uuid and fixed_player_uuid == self.uuid:
                    if debug_mode:
                        self.printer(f"[HISTORY] Skipping duplicate Hero action in update: {action_type} {action_amount}")
                else:
                    # Sanitiza round_state para usar UUIDs fixos
                    sanitized_round_state = self._sanitize_round_state(round_state)
                    ...
                    if debug_mode:
                        self.printer(f"[HISTORY] Recorded action: {seat_name} ({fixed_player_uuid[:8]}) {action_type} {action_amount}")
            else:
                # Se não conseguiu obter UUID, loga warning
                if debug_mode:
                    self.printer(f"[HISTORY] WARNING: Could not resolve UUID for player {seat_name}, action not recorded")
    ...
    def _get_fixed_uuid_from_seat(self, seat, debug_mode=False):
        ...
        if fixed_uuid:
            if debug_mode:
                self.printer(f"[DEBUG]   UUID mapeado: {seat_uuid_pypoker} -> {fixed_uuid} (bot: {seat_name})")
            return fixed_uuid
        ...
        if player_fixed_uuid:
            if debug_mode:
                self.printer(f"[DEBUG]   UUID mapeado: {seat_uuid_pypoker} -> {player_fixed_uuid} (jogador: {seat_name})")
            return player_fixed_uuid
        
        # Se não encontrou nenhum, retorna UUID original (pode ser jogador sem nome fixo)
        if debug_mode:
            self.printer(f"[DEBUG]   UUID não mapeado (mantendo original): {seat_uuid_pypoker}")
        return seat_uuid_pypoker
    ...
    def _get_player_cards(self, fixed_uuid, seat, all_cards, winners_with_cards, hand_info_dict, hand_info, seats, name, debug_mode=False):
        ...
        if hole_cards:
            if debug_mode:
                self.printer(f"[DEBUG]   Cartas encontradas no registry: {fixed_uuid}")
        ...
        if seat and isinstance(seat, dict):
            ...
            if seat_uuid and seat_uuid != fixed_uuid:
                ...
                if hole_cards:
                    if debug_mode:
                        self.printer(f"[DEBUG]   Cartas encontradas no registry (seat uuid): {seat_uuid}")
                    return normalize_hole_cards(hole_cards)
        
        # 2. Dos winners se este jogador é vencedor
        if fixed_uuid in winners_with_cards:
            if debug_mode:
                self.printer(f"[DEBUG]   Cartas encontradas nos winners: {fixed_uuid}")
        ...
        if fixed_uuid in hand_info_dict:
            if debug_mode:
                self.printer(f"[DEBUG]   Cartas encontradas no hand_info_dict: {fixed_uuid}")
            ...
            for item in hand_info:
                if isinstance(item, dict):
                    ...
                    if item_fixed_uuid == fixed_uuid or (not item_fixed_uuid and item_uuid == fixed_uuid):
                        if debug_mode:
                            self.printer(f"[DEBUG]   Cartas encontradas na busca direta do hand_info: {fixed_uuid}")
                        hole_card_from_info = (
        ...
        if hole_cards:
            if debug_mode:
                self.printer(f"[DEBUG]   Cartas encontradas no cache local: {fixed_uuid}")
            return normalize_hole_cards(hole_cards)
        ...
        if hole_cards:
            if debug_mode:
                self.printer(f"[DEBUG]   Cartas encontradas no seat: {fixed_uuid}")
                ...
                if isinstance(value, dict):
                    ...
                    if uuid_to_use:
                        ...
                        if debug_mode and fixed_uuid:
                            self.printer(f"[DEBUG]   hand_info mapeado: {uuid_from_info} -> {fixed_uuid} ({name_from_info})")
                    ...
                    if not name_from_info and uuid_from_info:
                        for seat in seats:
                            if isinstance(seat, dict) and seat.get('uuid') == uuid_from_info:
                                ...
                                if debug_mode:
                                    self.printer(f"[DEBUG]   Nome encontrado nos seats para UUID {uuid_from_info}: {name_from_info}")
                                break
    ...
    def _ensure_game_history_initialized(self, seats):
        ...
        if hasattr(self, 'uuid') and self.uuid:
            ...
            # Tenta encontrar nos seats pelo nome do jogador
            target_name = (self._player_name or "You").lower()
            for seat in seats:
                if isinstance(seat, dict) and seat.get('name', '').lower() == target_name:
                    player_uuid = seat.get('uuid')
        ...
        if player_uuid:
            try:
                ...
                debug_mode = os.environ.get('POKER_DEBUG', 'false').lower() == 'true'
                if debug_mode:
                    self.printer(f"[DEBUG] Erro ao inicializar histórico: {type(e).__name__}: {e}")
    ...
    def receive_round_result_message(self, winners, hand_info, round_state):
        ...
        try:
            ...
            if debug_mode:
                self.printer(f"[DEBUG] ========== receive_round_result_message CALLED ==========")
                self.printer(f"[DEBUG] Player UUID: {self.uuid}")
                self.printer(f"[DEBUG] Player Name: {self.name}")
            ...
            if hasattr(self, '_last_showdown_displayed') and self._last_showdown_displayed == round_id:
                if debug_mode:
                    self.printer(f"[DEBUG] Showdown already displayed for round {round_id}, skipping")
                return
            ...
            if seats:
                if debug_mode:
                    self.printer(f"[DEBUG] Processing {len(seats)} seats for card registration")
                for seat in seats:
                    if isinstance(seat, dict):
                        seat_uuid = seat.get('uuid', '')
                        seat_name = seat.get('name', '')
                        ...
                        if seat_uuid:
                            ...
                            if debug_mode:
                                self.printer(f"[DEBUG] Seat {seat_name} ({seat_uuid[:8]}): hole_cards={seat_hole_cards}")
                            if seat_hole_cards:
                                try:
                                    normalized_cards = normalize_hole_cards(seat_hole_cards)
                                    if normalized_cards:
                                        ...
                                        if fixed_uuid:
                                            ...
                                            if debug_mode:
                                                self.printer(f"[DEBUG] Cartas armazenadas do seat: {fixed_uuid} -> {normalized_cards}")
                                except Exception as e:
                                    if debug_mode:
                                        self.printer(f"[DEBUG] Erro ao armazenar cartas do seat: {e}")
            ...
            if debug_mode:
                self.printer(f"[DEBUG] ========== RAW hand_info ==========")
                self.printer(f"[DEBUG] Type: {type(hand_info)}")
                self.printer(f"[DEBUG] Content: {hand_info}")
                self.printer(f"[DEBUG] ====================================")
                        ...
                        if bot_hole_cards:
                            try:
                                ...
                                if normalized_cards:
                                    ...
                                    if debug_mode:
                                        self.printer(f"[DEBUG] Cartas armazenadas do hand_info: {fixed_uuid} ({player_name}) -> {normalized_cards}")
                            except Exception as e:
                                if debug_mode:
                                    self.printer(f"[DEBUG] Erro ao armazenar cartas do hand_info: {e}")
                ...
                if isinstance(seat, dict):
                    ...
                    if uuid_to_use and uuid_to_use not in hand_info_dict:
                        ...
                        if hole_cards:
                            ...
                            if debug_mode:
                                self.printer(f"[DEBUG] Added synthetic hand_info for {uuid_to_use} with cards {hole_cards}")
            # ==============================================================================

            if debug_mode:
                self.printer(f"[DEBUG] Registry tem {len(get_all_cards())} jogadores")
                self.printer(f"[DEBUG] hand_info_dict tem {len(hand_info_dict)} jogadores")
                self.printer(f"[DEBUG] winner_uuids: {winner_uuids}")
                self.printer(f"[DEBUG] ========== REGISTRY CONTENT ==========")
                all_cards_debug = get_all_cards()
                for uuid, cards in all_cards_debug.items():
                    self.printer(f"[DEBUG]   {uuid}: {cards}")
                self.printer(f"[DEBUG] ========================================")
            ...
            if winner_uuids or seats:
                    ...
                    try:
                        ...
                        if hole_cards and len(hole_cards) >= 2:
                            ...
                            if is_folded:
                                ...
                                self.printer(f"  {name}{position_indicator}: {cards_display} | {hand_desc}{all_in_indicator}")
                        else:
                ...
                if winner_uuids:
                    try:
                        ...
                        if player_won:
                            ...
                            self.printer()
                            self.printer(self.formatter.format_round_winner(round_number, "You", player_pot, player_hand_desc))
                            self.printer()
                    except Exception as e:
                        if debug_mode:
                            self.printer(f"[DEBUG] Erro ao processar winners: {type(e).__name__}: {e}")
                            import traceback
                            traceback.print_exc()
                ...
                try:
                    ...
                    if final_stacks:
                        self.printer(f"\nFinal stacks:")
                        self.printer(" | ".join(final_stacks))
                except Exception as e:
                    if debug_mode:
                        self.printer(f"[DEBUG] Erro ao mostrar stacks finais: {type(e).__name__}: {e}")
                    ...
                    try:
                        ...
                        if round_number >= 10:
                            try:
                                ...
                                if debug_mode:
                                    self.printer(f"[DEBUG] Erro ao salvar histórico: {type(e).__name__}: {e}")
                                self.printer(f"\n[Aviso: não foi possível salvar histórico]")
                    except Exception as e:
                        if debug_mode:
                            self.printer(f"[DEBUG] Erro ao registrar resultado: {type(e).__name__}: {e}")
                            import traceback
                            traceback.print_exc()
    ...
    def _get_current_street_bet(self, round_state):
        ...
        if debug_mode:
            self.printer(f"[DEBUG] _get_current_street_bet: No matching action found, returning 0")
        return 0
    ...
    def __receive_action_from_console(self, valid_actions, round_state=None, cached_player_stack=None) -> Tuple[str, int]:
        ...
        def _get_player_stack(round_state):
            ...
            if not round_state:
                if debug_mode:
                    self.printer(f"[DEBUG] _get_player_stack: round_state is None, returning 0")
                return 0
            ...
            if debug_mode:
                self.printer(f"[DEBUG] _get_player_stack: Searching for player stack...")
                self.printer(f"[DEBUG]   self.uuid: {getattr(self, 'uuid', 'NOT SET')}")
                self.printer(f"[DEBUG]   Number of seats: {len(seats)}")
            ...
            if hasattr(self, 'uuid') and self.uuid:
                for seat in seats:
                    if isinstance(seat, dict) and seat.get('uuid') == self.uuid:
                        player_seat = seat
                        if debug_mode:
                            self.printer(f"[DEBUG]   ✓ Found by UUID: stack={seat.get('stack', 0)}")
                        break
                ...
                for seat in seats:
                    if isinstance(seat, dict) and seat.get('name', '').lower() == target_name:
                        ...
                        if not hasattr(self, 'uuid') or not self.uuid:
                            self.uuid = seat.get('uuid')
                            if debug_mode:
                                self.printer(f"[DEBUG]   ✓ Found by name 'You', updated UUID: {self.uuid}")
                        if debug_mode:
                            self.printer(f"[DEBUG]   ✓ Found by name: stack={seat.get('stack', 0)}")
                        break
                        ...
                        if seat_hole_card:
                            ...
                            if seat_hole_cards == self.my_hole_cards:
                                ...
                                if not hasattr(self, 'uuid') or not self.uuid:
                                    self.uuid = seat.get('uuid')
                                if debug_mode:
                                    self.printer(f"[DEBUG]   ✓ Found by hole cards: stack={seat.get('stack', 0)}")
                                break
            ...
            if not player_seat and len(seats) == 1 and isinstance(seats[0], dict):
                player_seat = seats[0]
                if debug_mode:
                    self.printer(f"[DEBUG]   ⚠️  Using single seat fallback: stack={seats[0].get('stack', 0)}")
            
            if player_seat:
                stack = player_seat.get('stack', 0)
                if debug_mode:
                    self.printer(f"[DEBUG]   Final stack: {stack}")
                return stack
            
            if debug_mode:
                self.printer(f"[DEBUG]   ❌ ERROR: Could not find player seat! Returning 0")
                self.printer(f"[DEBUG]   Available seats:")
                for i, seat in enumerate(seats):
                    if isinstance(seat, dict):
                        self.printer(f"[DEBUG]     Seat {i}: name={seat.get('name')}, uuid={seat.get('uuid')}, stack={seat.get('stack')}")
            return 0
        ...
        try:
            ...
            if flg == 'a':
                ...
                debug_mode = os.environ.get('DEBUG_MODE', 'false').lower() == 'true'
                if debug_mode:
                    self.printer(f"[DEBUG] ALL-IN: player_stack={player_stack} (cached={cached_player_stack is not None}), current_bet={current_bet}, total={total_all_in}")
                    ...
                    if raise_action:
                        ...
                        if isinstance(raise_amount, dict):
                            ...
                            if final_amount >= effective_min or total_all_in <= effective_min:
                                ...
                                self.printer(f"⚠️  Seu stack total ({total_all_in}) é menor que o raise mínimo ({min_raise}). Fazendo all-in...")
                        ...
                        if call_action:
                            ...
                            self.printer("Erro: Não há ações válidas para all-in. Use outra ação.")
                            return self.__receive_action_from_console(valid_actions, round_state)
                else:
                    self.printer("Erro: Não foi possível determinar stack para all-in")
                    return self.__receive_action_from_console(valid_actions, round_state)
            ...
            if flg in self.__gen_valid_flg(valid_actions):
                if flg == 'f' and 'f' in action_map:
                    ...
                    debug_mode = os.environ.get('DEBUG_MODE', 'false').lower() == 'true'
                    if debug_mode:
                        self.printer(f"[DEBUG] CALL Logic: player_stack={player_stack} (cached={cached_player_stack is not None}), call_amount={call_amount}")
                        self.printer(f"[DEBUG] Condition check: player_stack > 0? {player_stack > 0}")
                        self.printer(f"[DEBUG] Condition check: call_amount > 0? {call_amount > 0}")
                        self.printer(f"[DEBUG] Condition check: player_stack <= call_amount? {player_stack <= call_amount}")
                    ...
                    if player_stack > 0 and call_amount > 0 and player_stack <= call_amount:
                        ...
                        if debug_mode:
                            self.printer(f"[DEBUG] Converting CALL to ALL-IN: returning ('raise', {total_all_in}) [Stack: {player_stack} + CurrentBet: {current_bet}]")
                        return 'raise', total_all_in
                    else:
                        # Stack suficiente ou call_amount é 0 (check)
                        if debug_mode:
                            self.printer(f"[DEBUG] Normal CALL: returning ('{action_data['action']}', {call_amount})")
            ...
            self.printer("Invalid action. Use [f] FOLD, [c] CALL, [r] RAISE, [a] ALL IN or [q] to quit")
            return self.__receive_action_from_console(valid_actions, round_state)
        except QuitGameException:
                ...
                if min_amount <= amount and amount <= max_amount:
                    ...
                    if round_state or cached_player_stack is not None:
                        ...
                        if player_stack > 0 and amount > max_possible_bet:
                            self.printer(f"⚠️  Valor excede seu stack total ({max_possible_bet}). Será all-in por {max_possible_bet}.")
                            return max_possible_bet
                            
                    return amount
                else:
                    self.printer(f"Invalid amount. Please enter a value between {min_amount} and {max_amount}")
                    return self.__receive_raise_amount_from_console(min_amount, max_amount, round_state, cached_player_stack)
            except ValueError:
                self.printer("Invalid input. Please enter a number.")
                return self.__receive_raise_amount_from_console(min_amount, max_amount, round_state, cached_player_stack)
        except QuitGameException:
            # Re-raise para ser capturado no nível superior
            raise
